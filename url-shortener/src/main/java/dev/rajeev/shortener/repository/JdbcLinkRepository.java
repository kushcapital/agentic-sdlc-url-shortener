...
public class JdbcLinkRepository implements LinkRepository {
    ...
    public void insert(Link link) {
        try {
            jdbc.update("INSERT INTO links(code, target_url, created_at, deleted_at, custom, owner_id) VALUES (?, ?, ?, ?, ?, ?)",
                    link.code(), link.targetUrl(), ts(link.createdAt()), ts(link.deletedAt()), link.custom(), link.ownerId());
    ...
    public Optional<Link> findByCode(String code) {
        return jdbc.query("SELECT code, target_url, created_at, deleted_at, custom, owner_id FROM links WHERE code = ?", LINK_ROW, code)
                .stream().findFirst();
    }
